...
- **IMPLEMENTED BEHAVIOR:** Production had the management units but no vehicle or implement `managementUnitId` links. Among matching vehicle codes, 1,543 had a different `complexCode` from local. The one-time sync matched by unique code and copied only `complexCode`, `unit`, `assignedUnitCode`, and `managementUnitId` for vehicles, and `unit`, `assignedUnitCode`, and `managementUnitId` for implements. It changed 2,344 production vehicles and 938 implements. It also created 46 local-only vehicles after checking code/plate collisions and mapping the vehicle type and active management unit. A final dry run reported zero differences, and linked vehicle/implement counts agree for every local management unit. Pre-change JSON snapshots are in the private `output/` directory.
- **GAP:** 687 vehicle and 64 implement codes exist only on production. They remain intact. The sync did not delete assets, change operational history, or copy driver assignments. Rows that differ in name despite sharing a code were left with their production names.
- **RECOMMENDED CHANGE:** Review production-only asset codes and driver assignment history separately before any removal or reassignment. Keep code uniqueness enforced by the database and avoid matching by display name.
