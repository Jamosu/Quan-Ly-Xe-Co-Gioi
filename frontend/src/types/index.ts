...
export type TransportWorkflowStatus = 'DRAFT' | 'PENDING_APPROVAL' | 'APPROVED' | 'ASSIGNED' | 'DRIVER_ACCEPTED' | 'AT_PICKUP' | 'LOADING' | 'DEPARTED' | 'IN_TRANSIT' | 'AT_DELIVERY' | 'UNLOADING' | 'DELIVERED' | 'ACCEPTED' | 'COMPLETED' | 'CANCELLED';
...
export interface DispatchOrderRecord { id: number; code: string; sourceType: string; unit: string; purpose: string; origin: string; destination: string; departureTime?: string; plannedEndTime?: string; actualDepartureTime?: string; status: DispatchWorkflowStatus; isDelayed: boolean; notes?: string; planNotes?: string; taskNotes?: string; orderCategory?: string; categoryLabel?: string; workVolumeTarget?: number; workVolumeUnit?: string; plannedFuelLiters?: number; fuelQuotaRate?: string; vehicle?: ApiVehicle; driver?: ApiPerson; implement?: { id: number; code: string; name: string }; legacyVehicle?: string; legacyDriver?: string; approvedAt?: string; confirmations?: OperationConfirmationRecord[]; }
