...
import { Layers, Tractor, HardHat, Truck } from 'lucide-react';
import { operationsApi } from '../../api/operations';

export type DispatchCategoryTabKey = 'ALL' | 'NONG_NGHIEP' | 'CONG_TRINH' | 'VAN_CHUYEN';
    ...
    const loadCounts = async () => {
      try {
        const [dispatchRes, transportRes] = await Promise.allSettled([
          ...
          operationsApi.transportOrders({ limit: 1000 }),
        ]);
        ...
        const totalCount = nongNghiepCount + congTrinhCount + vanChuyenCount;
    ...
    <div className="flex flex-wrap items-center gap-1.5 rounded-2xl border border-slate-200 bg-white p-1.5 shadow-xs">
      {tabs.map((tab) => {
        ...
        return (
          ...
          >
            <Icon className={`h-4 w-4 ${isActive ? 'text-amber-400' : tab.color}`} />
            <span>{tab.label}</span>
            <span
