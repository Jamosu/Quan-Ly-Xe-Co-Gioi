...
export const Modal: React.FC<ModalProps> = ({
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto bg-slate-900/60 backdrop-blur-sm transition-opacity duration-200 cursor-pointer"
      onClick={onClose}
    >
      {/* Modal Dialog */}
      <div
        onClick={(e) => e.stopPropagation()}
        className={`relative w-full ${sizeClasses[size]} bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden transform transition-all z-10 animate-in fade-in zoom-in-95 duration-150 cursor-default`}
      >
        {/* Header - Chuẩn xanh THACO AGRI */}
        <div className="flex items-center justify-between px-6 py-4 bg-emerald-800 text-white shrink-0">
          <div>
            ...
            {subtitle && <p className="text-xs text-emerald-100/90 mt-0.5">{subtitle}</p>}
          </div>
          <button
            type="button"
            onClick={onClose}
