...
export class InternalFeedService {
  ...
  async createTrip(dto: CreateFeedTripDto, actor: OperationalActor) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const workOrder = await tx.operationalWorkOrder.create({ data: { type: WorkOrderType.INTERNAL_FEED, unit: vehicle.unit, category: 'TRANSPORT', sourceType: 'MANUAL', jobName: `Tiếp liệu ${trip.code}`, workLocationText: trip.destinationLocation, assignmentMode: WorkAssignmentMode.FIXED_ASSIGNMENT, status: WorkOrderStatus.ASSIGNED, plannedStartAt: dto.slaWindowStart, plannedEndAt: dto.slaWindowEnd, internalFeedTripId: trip.id, createdById: actor.id } });
