...
async function main() {
  const [localUnits, productionUnits, localVehicles, productionVehicles, localImplements, productionImplements] = await Promise.all([
    ...
    local.vehicle.findMany({ select: { code: true, name: true, complexCode: true, unit: true, assignedUnitCode: true, managementUnitId: true } }),
    production.vehicle.findMany({ select: { code: true, name: true, complexCode: true, unit: true, assignedUnitCode: true, managementUnitId: true } }),
  ...
  console.log(JSON.stringify({ mode: process.argv.includes('--apply') ? 'apply' : 'dry-run',
    vehicles: { ...vehicles, changes: vehicles.changes.length }, implements: { ...implementsResult, changes: implementsResult.changes.length },
  ...
  fs.writeFileSync(snapshotPath, JSON.stringify({ vehicles: vehicles.changes, implements: implementsResult.changes }, null, 2));
  ...
  console.log(JSON.stringify({ applied: true, snapshotPath, vehiclesUpdated: vehicles.changes.length, implementsUpdated: implementsResult.changes.length }));
}
